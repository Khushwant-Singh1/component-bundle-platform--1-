...
export const bundleSchema = z.object({
  name: z.string().min(1, "Bundle name is required").max(100, "Bundle name must be less than 100 characters"),
  ...
  techStack: z.array(z.string()).min(1, "At least one technology is required").max(15, "Maximum 15 technologies allowed"),
  features: z.array(z.string().min(1, "Feature cannot be empty")).min(1, "At least one feature is required").max(20, "Maximum 20 features allowed"),
  includes: z.array(z.string().min(1, "Include item cannot be empty")).min(1, "At least one include item is required").max(20, "Maximum 20 include items allowed"),
...
export const bundleFormSchema = z.object({
  ...
  setupTime: z.string().optional(),
  estimatedValue: z.string().optional(),
  demoUrl: z.string().optional().refine((val) => !val || val === "" || z.string().url().safeParse(val).success, "Please enter a valid URL"),
  githubUrl: z.string().optional().refine((val) => !val || val === "" || z.string().url().safeParse(val).success, "Please enter a valid URL"),
  isActive: z.boolean().default(true),
  isFeatured: z.boolean().default(false),
  isBestseller: z.boolean().default(false),
  tags: z.array(z.string()).min(1, "At least one tag is required").max(10, "Maximum 10 tags allowed"),
